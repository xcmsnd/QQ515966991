...
Simple.Text = "SimpleSpy For Mobile [Aser翻译]"
...
newButton("获取脚本", function()
	return "单击以将调用脚本复制到剪贴板\n警告：不是超级可靠，nil=找不到"
end, function()
	if selected then
...
newButton("阻止(i)", function()
	return "单击以停止此远程激发。\n阻止远程不会将其从SimpleSpy日志中删除，但它不会继续激发服务器。"
end, function()
	if selected then
...
newButton("阻止 (n)", function()
	return "单击以停止激发具有此名称的远程。\n阻止远程不会将其从SimpleSpy日志中删除，但不会继续激发服务器。"
end, function()
	if selected then
...
--- clears blacklist
newButton("清除阻止进程", function()
	return "单击以停止阻止远程。\n阻止远程不会将其从SimpleSpy日志中删除，但不会继续启动服务器。"
end, function()
	blocklist = {}
